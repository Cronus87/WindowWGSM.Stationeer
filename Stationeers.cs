...
namespace WindowsGSM.Plugins
{
    public class ARMA3 : SteamCMDAgent // SteamCMDAgent is used because ARMA3 relies on SteamCMD for installation and update process
    {
        ...
        public override bool loginAnonymous => true; // ARMA3 requires to login steam account to install the server, so loginAnonymous = false
        public override string AppId => "600760"; // Game server appId, ARMA3 is 233780


        // - Game server Fixed variables
        public override string StartPath => "rocketstation_DedicatedServer.exe"; // Game server start path, for ARMA3, it is arma3server.exe
